...
- 羽田第3・京急: 品川、大門
- 羽田第3・東京モノレール: 浜松町
- 成田第2・京成スカイライナー: 青砥、日暮里
- 成田第2・JR成田エクスプレス: 東京、新宿
- 成田第2・京成アクセス特急: 青砥、実際に通る押上または日暮里
- 成田第2・京成本線系統: 青砥、実際に通る押上または日暮里

成田第2の京成サンプルではSourceを`keisei-nrt-t2`のまま維持し、Departureの`routeName`と
`via`で成田スカイアクセス線・京成本線の経路差を表現する。実データ取得元で路線・ホームを
安定して判別できる場合は、`trackGroup`や`platform`を追加するか、Sourceを
`keisei-skyliner-nrt-t2`、`keisei-access-nrt-t2`、`keisei-mainline-nrt-t2`などへ分割する。
現時点ではデータ取得元の構造に合わせ、無理なSource分割は行わない。
...
成田第2は京成とJRの2 Sourceを持ち、`nrt_t2_weekday.json`と
`nrt_t2_holiday.json`に同一のサンプルDepartureを格納している。実データではない。
...
エアポートバス東京・成田は、公式PDFに記載された成田空港第2ターミナル発と東京駅・銀座駅の
